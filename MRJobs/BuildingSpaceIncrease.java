...
public class BuildingSpaceIncrease {
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = new Job(conf, "building course and capacity in every semester");
		job.setJarByClass((Class) BuildingSpaceIncrease.class);
		...
		Job job2 = new Job(conf2, "building capacity in every year one key");
		job2.setJarByClass((Class) BuildingSpaceIncrease.class);
		...
		Job job3 = new Job(conf3, "building capacity increase in every year range");
		job3.setJarByClass((Class) BuildingSpaceIncrease.class);
	...
	public static class Mapper3 extends Mapper<Object, Text, Text, IntWritable> {
		...
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			String hallName = prevKeyTerms[1];

			StringBuilder hallYearKey1 = new StringBuilder();
			StringBuilder hallYearKey2 = new StringBuilder();
			hallYearKey1.append(Integer.toString(((Integer.parseInt(year)) - 1)) + "-" + year + " " + hallName);
			hallYearKey2.append(year + "-" + Integer.toString(((Integer.parseInt(year)) + 1)) + " " + hallName);

			word.set(hallYearKey1.toString());
			context.write(word, new IntWritable(capacity));

			word.set(hallYearKey2.toString());
			context.write(word, new IntWritable(capacity));
		}
